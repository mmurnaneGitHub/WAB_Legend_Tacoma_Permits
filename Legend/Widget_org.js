  ...
  var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    startup: function () {
      ...
      /* MJM - Add feature layer to map --------------------------------------------------------------
        CivicData Permits Table: http://www.civicdata.com/dataset/mapdata_v3_17678/resource/9474d5f7-fac8-451a-8e48-93a9ae6d6077
        JSON Call (works, text in browser, need limit variable or else get only the first 100 records): 
          http://www.civicdata.com/api/3/action/datastore_search?resource_id=9474d5f7-fac8-451a-8e48-93a9ae6d6077&limit=1000000

        API Call: http://www.civicdata.com/api/action/datastore_search?resource_id=9474d5f7-fac8-451a-8e48-93a9ae6d6077
        OTHER JSON Call (text in browser, first 100 records): http://www.civicdata.com/api/3/action/datastore_search?resource_id=9474d5f7-fac8-451a-8e48-93a9ae6d6077
        OTHER JSON Call (download file in browser): http://www.civicdata.com/datastore/json/9474d5f7-fac8-451a-8e48-93a9ae6d6077/
        CSV Call: http://www.civicdata.com/datastore/dump/9474d5f7-fac8-451a-8e48-93a9ae6d6077
    ...
    */
      ...
      var categoryStore = new Memory({
        data: [
          ...
          { name: "Emergency Management", id: "Emergency" },
          { name: "Fire", id: "Fire" },
    ...
    _queryROW: function (json1_ALL_tmp) {
      ...
      array.forEach(dijit.byId("ROWMenu").value, function (item) {  //loop through the items (SELECTED MENU ITEMS)
        ...
        if (item == 'Power') {
          theROWList1 = json1_ALL_tmp.filter(function (row) {
            return row['Permit_Type_Description'] == 'ePermit Tacoma Power Utility ';  //menu value has a space on end
          });
        }
        if (item == 'Transmission') {
          theROWList1 = json1_ALL_tmp.filter(function (row) {
            return row['Permit_Type_Description'] == 'ePermit Environmental Services Utility ';  //menu value has a space on end
          });
        }
        if (item == 'Tree') {
        ...
        if (item == 'Water') {
          theROWList1 = json1_ALL_tmp.filter(function (row) {
            return row['Permit_Type_Description'] == 'ePermit Tacoma Water Utility ';  //menu value has a space on end
          });
        }
      });
    ...
    _queryPermits: function (lowerLeftLL, upperRightLL, startDate, endDate) {
      ...
      if (dijit.byId("categorySelect").value == 'All') { //excluding some categories from map
      ...
      } else if (dijit.byId("categorySelect").value == 'Emergency') {
        json1_ALL_tmp = json1_ALL.filter(function (row) {
          return row['Longitude'] > lowerLeftLL['0'] && row['Longitude'] < upperRightLL['0'] && row['Latitude'] > lowerLeftLL['1'] && row['Latitude'] < upperRightLL['1'] && Date.parse(row['Applied_Date']) <= Date.parse(endDate) && Date.parse(row['Applied_Date']) >= Date.parse(startDate) && (row['Permit_Type_Description'] == 'Additional Services Flood Plain '); //menu value has a space on end
        });
        ...
        json1_ALL_tmp = json1_ALL.filter(function (row) {
          return row['Longitude'] > lowerLeftLL['0'] && row['Longitude'] < upperRightLL['0'] && row['Latitude'] > lowerLeftLL['1'] && row['Latitude'] < upperRightLL['1'] && Date.parse(row['Applied_Date']) <= Date.parse(endDate) && Date.parse(row['Applied_Date']) >= Date.parse(startDate) && (row['Permit_Type_Description'] == 'Right-of-Way Construction ' || row['Permit_Type_Description'] == 'Right-of-Way Occupancy ' || row['Permit_Type_Description'] == 'Right-of-Way Tree ' || row['Permit_Type_Description'] == 'Right-of-Way Use ' || row['Permit_Type_Description'] == 'Right-of-Way Utility ' || row['Permit_Type_Description'] == 'ePermit Tacoma Power Utility ' || row['Permit_Type_Description'] == 'ePermit Environmental Services Utility ' || row['Permit_Type_Description'] == 'ePermit Tacoma Water Utility '); //menu value has a space on end
        });
        //Determine if subcategories are needed 
    ...
    _queryPermitsPolygon: function (json1_POLYGON, startDate, endDate) {
      ...
      if (dijit.byId("categorySelect").value == 'All') { //excluding some categories from map
        ...
        json1_ALL_tmp = json1_ALL.filter(function (row) {
          return Date.parse(row['Applied_Date']) <= Date.parse(endDate) && Date.parse(row['Applied_Date']) >= Date.parse(startDate) && (row['Permit_Type_Description'] == 'Additional Services Flood Plain '); //menu value has a space on end        });
        });
      } else if (dijit.byId("categorySelect").value == 'Fire') {
        json1_ALL_tmp = json1_POLYGON.filter(function (row) {
        ...
        json1_ALL_tmp = json1_POLYGON.filter(function (row) {
          return Date.parse(row['Applied_Date']) <= Date.parse(endDate) && Date.parse(row['Applied_Date']) >= Date.parse(startDate) && (row['Permit_Type_Description'] == 'Right-of-Way Construction ' || row['Permit_Type_Description'] == 'Right-of-Way Occupancy ' || row['Permit_Type_Description'] == 'Right-of-Way Tree ' || row['Permit_Type_Description'] == 'Right-of-Way Use ' || row['Permit_Type_Description'] == 'Right-of-Way Utility ' || row['Permit_Type_Description'] == 'ePermit Tacoma Power Utility ' || row['Permit_Type_Description'] == 'ePermit Environmental Services Utility ' || row['Permit_Type_Description'] == 'ePermit Tacoma Water Utility '); //menu value has a space on end
        });
        //Determine if subcategories are needed 
        if (dijit.byId("ROWMenu").value.length > 0 && dijit.byId("ROWMenu").value.length < dijit.byId("ROWMenu").options.length) { //determine how many submenu items checked (between none or all (options))
          json1_ALL_tmp = this._queryROW(json1_ALL_tmp); //need to process further for ROW subcategory selection
